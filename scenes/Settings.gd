...
func _ready():
	if(screenSizes < 2):
		get_node("Panel/OptionButton").add_item("Windowed")
		get_node("Panel/OptionButton").add_item("Fullscreen")
		screenSizes+=2
...
func _on_MenuMusic_value_changed(value):
	$Panel/Menu_Level.text = str(value)


func _on_GameMusic_value_changed(value):
	$Panel/Game_Level.text = str(value)
